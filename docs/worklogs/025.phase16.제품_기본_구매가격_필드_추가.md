...
### Products 목록 페이지에서 구매가격 미표시 문제 (2024-12-04)

**문제**: Products 목록 페이지에서 `default_purchase_price`를 입력해도 목록에 가격이 표시되지 않음

**원인**: `app/(dashboard)/products/page.tsx`에서 Supabase 쿼리 시 `default_purchase_price` 컬럼이 select에 누락됨

**수정 파일**: `app/(dashboard)/products/page.tsx`
...
**커밋**: `b52f1a6` - Fix: Products 목록 페이지에서 default_purchase_price 컬럼 조회 추가
...
| 커밋 | 설명 |
|------|------|
| `7d8eb96` | Feat: 제품 기본 구매가격 필드 추가 |
| `b52f1a6` | Fix: Products 목록 페이지에서 default_purchase_price 컬럼 조회 추가 |
